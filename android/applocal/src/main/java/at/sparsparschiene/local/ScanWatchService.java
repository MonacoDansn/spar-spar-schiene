...
/**
 * Lokale Variante der Fortschritts-Benachrichtigung: pollt den eingebetteten
 * Server (127.0.0.1, kein Auth) und haelt als Foreground-Service zugleich den
 * Prozess - und damit den laufenden Scan - bei Display-aus am Leben.
 */
public class ScanWatchService extends Service {
    ...
    private static final int NOTIF_DONE_ID = 2;  // Abschluss - ueberlebt den Service-Tod
    private static final int POLL_MS = 5_000;    // lokal darf oefter gepollt werden
    private static final long MAX_RUNTIME_MS = 60 * 60_000L;
        ...
        while (worker == me && !me.isInterrupted()
            ...
            try {
                HttpURLConnection c = (HttpURLConnection) new URL(
                        LocalMainActivity.BASE_URL + "/api/scan/" + jobId + "?light=1")
                        .openConnection();
                ...
                if (worker != me) return;
                if (status == 404) {
                    finish("⚠️ Scan nicht mehr vorhanden (App wurde beendet?)");
                    return;
                }
                if (status == 200) {
                ...
                // Server startet evtl. gerade erst: naechster Versuch
            }
    ...
    private Notification build(String text, int done, int total, boolean ongoing) {
        Intent open = new Intent(this, LocalMainActivity.class);
        PendingIntent pi = PendingIntent.getActivity(this, 0, open,
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        Notification.Builder b = new Notification.Builder(this, CHANNEL)
                .setSmallIcon(R.drawable.ic_fg)
                .setContentTitle("Spar Spar Schiene Lokal")
                .setContentText(text)
